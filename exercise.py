...
def calculate_dog_years():
    ...
    try:
        ...
        if dog_age < 0:
            print("Invalid input. A dog's age cannot be negative.")
            return
...
def weather_advice():
    # Your control flow logic goes here
    is_cold = input("Is it cold? (yes/no): ").strip()
...
def determine_season():
    ...
    if month not in month_to_number:
        print("Invalid month entered. Please use three-character format (e.g., Jan, Feb).")
        return

    try:
        day = int(input("Enter the day of the month: ").strip())
    except ValueError:
        print("Invalid day entered. Please enter a numeric value.")
        return
